...
impl RawSocket {
    ...
    pub fn send_with_buf(&self, payload: &[u8], buf: &mut Vec<u8>) -> io::Result<usize> {
        let result = self.0.send_with_buf(payload, buf);
        // Log the first few send errors to avoid log spam
        if let Err(ref e) = result {
            static SEND_ERR_COUNT: std::sync::atomic::AtomicU32 = std::sync::atomic::AtomicU32::new(0);
